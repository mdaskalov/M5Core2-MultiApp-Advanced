...
void WebRadioClass::getvolume()
{
	...
	menu.vol = preferences.getFloat("vol", 15.0f);
	preferences.end();
}
...
void WebRadioClass::Run()
{
	M5.update();
	getvolume();
	menu.drawAppMenu(F("WebRadio"), F("Vol-"), F("Next"), F("Vol+"));
	M5.Lcd.setTextColor(ORANGE);
	...
	if (GetStations(My_SD, "/RadioStations.txt"))
	{
		if (WiFi.isConnected())
		{
			...
			while (play)
			{
				...
				if (M5.BtnA.wasPressed() && menu.vol > 0)
				{
					menu.vol -= 5;
					setVolume(&menu.vol);
				}
				if (M5.BtnC.wasPressed() && menu.vol < 100)
				{
					menu.vol += 5;
					setVolume(&menu.vol);
				}
				...
				if (menu.vol != menu.old_vol)
				{
					menu.HprogressBar(80, 170, 200, 15, GREEN, menu.vol, true);
					menu.old_vol = menu.vol;
				}
				M5.update();
				...
				if (upd)
				{
					...
					menu.HprogressBar(80, 150, 200, 15, RED, 0, true);
					rawFillLvl = 0;
				...
				else
				{
					...
					if (rawFillLvl != buff->getFillLevel())
					{
						...
						menu.HprogressBar(80, 150, 200, 15, RED, fillLvl, true);
					}
					if (!upd && !fillLvl)
					{
						StopPlaying();
			...
			preferences.begin("Volume", false);
			preferences.putFloat("vol", menu.vol);
			preferences.end();
			StopPlaying();
			if (out)
			{
	...
	{
		M5.Lcd.setTextColor(WHITE);
		M5.Lcd.drawCentreString("RadioStations.txt Not Found on SD", 160, 60, 2);
		delay(3000);
	}
}
